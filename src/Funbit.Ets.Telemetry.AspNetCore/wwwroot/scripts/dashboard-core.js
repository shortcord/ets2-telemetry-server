        ...
        (function (Telemetry) {
            ...
            var Dashboard = (function () {
                function Dashboard(telemetryEndpointUrl, skinConfig) {
                    ...
                    this.hubConnectionStopping = false;
                ...
                Dashboard.prototype.logError = function (message, data) {
                    if (window.console && typeof console.error === 'function') {
                        if (typeof data !== 'undefined') {
                            console.error('[TelemetryDashboard] ' + message, data);
                        } else {
                            console.error('[TelemetryDashboard] ' + message);
                        }
                    }
                };

                Dashboard.prototype.normalizeTelemetryObject = function (value) {
                    var _this = this;
                    if (value == null) {
                        return value;
                    }
                    if ($.isArray(value)) {
                        return value.map(function (item) {
                            return _this.normalizeTelemetryObject(item);
                        });
                    }
                    ...
                    Object.keys(value).forEach(function (key) {
                        ...
                        normalized[normalizedKey] = _this.normalizeTelemetryObject(value[key]);
                    });
                    return normalized;
                };
                ...
                Dashboard.prototype.initializeHub = function () {
                    ...
                    window.onbeforeunload = function () {
                        _this.hubConnectionStopping = true;
                        if (_this.hubConnection) {
                            _this.hubConnection.stop();
                        }
                    };
                };
                ...
                Dashboard.prototype.connectToHub = function () {
                    var _this = this;
                    this.hubConnectionStopping = false;
                    var builder = new signalR.HubConnectionBuilder().withUrl(Telemetry.Configuration.getUrl('/signalr'));
                    if (typeof builder.withAutomaticReconnect === "function") {
                        builder = builder.withAutomaticReconnect([0, 2000, 5000, 10000, 30000]);
                    }
                    if (typeof builder.configureLogging === "function") {
                        builder = builder.configureLogging(signalR.LogLevel.None);
                    }

                    this.hubConnection = builder.build();
                    this.hubConnection.on('updateData', function (json) {
                    ...
                    this.hubConnection.onclose(function () {
                        if (!_this.hubConnectionStopping) {
                    ...
                    this.hubConnection.start().then(function () {
                        ...
                        _this.logError('Failed to start hub connection');
                ...
                Dashboard.prototype.requestDataUpdate = function () {
                    ...
                    if (this.hubConnection && this.hubConnection.state === signalR.HubConnectionState.Connected) {
                        this.hubConnection.invoke('RequestData').catch(function (err) {
                            if (window.console && typeof console.error === 'function') {
                                console.error('[TelemetryDashboard] RequestData invocation failed', err);
                            }
                        });
                    }
                };
                ...
                Dashboard.prototype.dataUpdateCallback = function (jsonData) {
                    try {
                        ...
                        data = this.normalizeTelemetryObject(data);
                        this.process(data);
                        this.requestDataUpdate();
                    } catch (err) {
                        this.logError('Failed to process websocket payload', {
                            error: err,
                            payload: jsonData
                        });
                    }
                ...
                Dashboard.prototype.process = function (data, reason) {
                    ...
                    try {
                        ...
                        this.logError('filter() threw', err);
                        throw err;
                    }
                    ...
                    try {
                        ...
                        this.logError('internalFilter() threw', err);
                        throw err;
                    }

                    this.lastDataRequestFrameDiff = this.frame - this.lastDataRequestFrame;
                    if (this.latestData == null) {
                        // Prime the renderer so the first telemetry packet can paint immediately.
                        this.prevData = data;
                        this.frameData = data;
